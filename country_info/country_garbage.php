...
function updater($value) {
   ...
   $sql = "UPDATE Staff_info SET Status = '正常' WHERE Name = '$value'";
   $update = $conn->prepare($sql);
   
   $update->execute();
   if ($update->affected_rows > 0) {
       header("Location: staff_garbage.php");
	   echo "success";
   }else {
	...
	$result = mysqli_query($con,"SELECT * FROM Staff_info WHERE Status = '刪除'");
...
<body>
    ...
    <p style='margin-top:50px;margin-bottom:50px;' >
        <a href="country_info.php" class="btn btn-warning">員工資料總表</a>
	...
	<div align="center">
		...
		輸入欲重設的姓名：<input  type="text" name="Name" required="required">
		<input  type="submit" class='btn btn-primary' value = "復原"/><br/>
		</form>
	</div>
